...
class SessionStatsDisplay:
    ...
    def _display_overall_stats(self, stats: Dict):
        ...
        print(f"├─ Total Value: {value_color}{stats['total_value']:.2f} exalted{Colors.END}")
    ...
    def _display_efficiency_stats(self, stats: Dict):
        ...
        print(f"├─ Value per Hour: {value_per_hour_color}{stats['value_per_hour']:.2f} exalted{Colors.END}")
        print(f"├─ Avg Map Value: {Colors.GOLD}{stats['avg_map_value']:.2f} exalted{Colors.END}")
    ...
    def _display_averages(self, stats: Dict):
        ...
        print(f"├─ Avg Session Value: {Colors.GOLD}{stats['avg_session_value']:.2f} exalted{Colors.END}")
    ...
    def _display_best_session(self, stats: Dict):
        ...
        print(f"│  ├─ Value: {Colors.GOLD}{best.total_value:.2f} exalted{Colors.END}")
        ...
        if best_maps.session_id != best.session_id:
            ...
            print(f"   ├─ Value: {Colors.GOLD}{best_maps.total_value:.2f} exalted{Colors.END}")
    ...
    def _display_character_breakdown(self):
        ...
        for i, (char, cstats) in enumerate(char_stats.items()):
            ...
            print(f"{sub_prefix}├─ Value: {value_color}{cstats['total_value']:.2f} exalted{Colors.END}")
            print(f"{sub_prefix}├─ Maps: {Colors.GREEN}{cstats['total_maps']}{Colors.END}")
            print(f"{sub_prefix}└─ Efficiency: {efficiency_color}{cstats['maps_per_hour']:.2f} maps/h{Colors.END}, {Colors.GOLD}{cstats['value_per_hour']:.2f} exalted/h{Colors.END}")
    ...
    def _display_top_sessions(self):
        ...
        for i, session in enumerate(top_value):
            ...
            print(f"│  {i+1}. {Colors.CYAN}{date_str}{Colors.END} | {value_color}{session.total_value:.2f} exalted{Colors.END} | {Colors.GREEN}{session.total_maps} maps{Colors.END} | {Colors.GRAY}{runtime_str}{Colors.END}")
        ...
        for i, session in enumerate(top_maps):
            ...
            print(f"   {i+1}. {Colors.CYAN}{date_str}{Colors.END} | {maps_color}{session.total_maps} maps{Colors.END} | {Colors.GOLD}{session.total_value:.2f} exalted{Colors.END} | {Colors.GRAY}{runtime_str}{Colors.END}")
    ...
    def _display_recent_activity(self):
        ...
        print(f"├─ Value: {Colors.GOLD}{total_recent_value:.2f} exalted{Colors.END}")
    ...
    def display_daily_breakdown(self, days: int = 7):
        ...
        for day in recent_days:
            ...
            print(f"{day_color}{day}{Colors.END} | "
                  ...
                  f"{Colors.GOLD}{stats['total_value']:.2f} exalted{Colors.END} | "
