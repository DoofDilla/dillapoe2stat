...
class NotificationManager:
    ...
    def __init__(self, config, templates=None):
        ...
        # Enable notifications only on Windows and if library is available
        self.notifications_enabled = (
            self.config.NOTIFICATION_ENABLED and
    ...
    def notify_from_template(self, template_key, icon_type=None, **values):
        ...
        if not self.notifications_enabled:
            # Optional fallback: Print to console for visibility on Linux
            print(f"[Notification Skipped - Linux Mode] {template_key}: {values}")
            return
